...
// Deserialization
		...
		try {
			...
			bt = new JButton("Jogar"); // Panel to add buttons is created
			JPanel p = new JPanel(); // Add buttons and text field to panel
			p.add(bt);
			p.add(lb); // set the background color of panel
			p.setBackground(Color.DARK_GRAY); // add panel to frame
			fr.add(p);
